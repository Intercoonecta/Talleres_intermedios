...
Lo importante en todos los casos es la consistencia, y sobretodo no invertir el orden del mes y el día ni escribir indistintamente "Sep", "sep" o "Septiembre" . Es sumamente importante revisar esto antes de importar los datos en R.

![](fechas.jpg){fig-align="center" width="266"}

Al importar en R desde archivos ascii con la función `read.table()`, la clase de la variable con la fecha será `character`, lo cual dificultará su uso en gráficos o para realizar operaciones, por lo que debemos convertirlos a una clase especial para fechas o fecha-tiempo. Cuando se importan archivos de Excel con la función `read_excel()` del paquete **readxl** usualmente ya pasa a una clase A continuación se describen algunas de estas clases.
