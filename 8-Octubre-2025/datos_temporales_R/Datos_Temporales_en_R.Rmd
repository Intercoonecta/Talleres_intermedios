...
Entonces, considerando que al 1 de febrero de 1970 transcurrieron 31 días desde el origen mencionado antes,
...
Inspeccionando un poco el archivo (tecleando ncf en la consola) podemos notar que estos números son: "units: seconds since 1970-01-01 00:00:00", por lo que la conversión apropiada sería
...
Para graficar la serie de tiempo de temperatura potencial del mar a 0.49 m de profundidad en el punto anterior usamos la función `plot()`.

```{r}
plot(tsm, type = "o", pch = 16, col = rgb(1, 0, 0, 0.2))
```

Esta serie se puede suavizar con promedios móviles centrados mediante la función `cma()` del paquete **smooth** que tiene la ventaja de no producir valores faltantes
...
Podemos simplificar aún más esta información, por ejemplo calculando los promedios mensuales. Para ello primero extraemos el mes y el año de la fecha
...
Si quisiéramos personalizar el espaciamiento de las marcas de graduación en x, por ejemplo cada mes
